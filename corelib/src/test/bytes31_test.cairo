...
fn test_bytes31_to_from_felt252() {
    ...
    assert!(zero_as_bytes31.is_some());
    assert!(zero_as_bytes31.unwrap().into() == 0_felt252);

    let one_as_bytes31: Option<bytes31> = 1.try_into();
    assert!(one_as_bytes31.is_some());
    assert!(one_as_bytes31.unwrap().into() == 1_felt252);

    let max_as_bytes31: Option<bytes31> = (POW_2_248 - 1).try_into();
    assert!(max_as_bytes31.is_some());
    assert!(max_as_bytes31.unwrap().into() == POW_2_248 - 1);
...
fn test_u8_into_bytes31() {
    ...
    assert!(one_as_bytes31.into() == 1_felt252);

    let max_u8 = 0xff_u8;
    let max_as_bytes31: bytes31 = max_u8.into();
    assert!(max_as_bytes31.into() == 0xff_felt252);
}
...
fn test_u16_into_bytes31() {
    ...
    assert!(max_as_bytes31.into() == 0xffff_felt252);
}
...
fn test_u32_into_bytes31() {
    ...
    assert!(max_as_bytes31.into() == 0xffffffff_felt252);
}
...
fn test_u64_into_bytes31() {
    ...
    assert!(max_as_bytes31.into() == 0xffffffffffffffff_felt252);
}
